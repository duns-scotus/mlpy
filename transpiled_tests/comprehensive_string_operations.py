...
def string_creation_basics():
    ...
    print((str('Empty string length: ') + str(ml_string.length(empty_str))))
    print((str('Simple string: ') + str(simple_str)))
    print((str('Full name: ') + str(full_name)))
    print((str('Greeting: ') + str(greeting)))
    age = 25
    age_str = (str((str('I am ') + str(age))) + str(' years old'))
...
def string_length_and_access():
    ...
    if (length > 0):
        ...
        middle_char = ml_string.char_at(text, (length / 2))
    ...
    print((str("Character code for 'A': ") + str(a_code)))
    print((str("Character code for 'z': ") + str(z_code)))
...
def string_search_operations():
    ...
    print((str("Contains 'fox': ") + str(contains_fox)))
    print((str("Contains 'cat': ") + str(contains_cat)))
    print((str("Starts with 'The': ") + str(starts_with_the)))
    print((str("Ends with 'dog': ") + str(ends_with_dog)))
    fox_position = ml_string.find(text, 'fox')
    the_position = ml_string.find(text, 'the')
    missing_position = ml_string.find(text, 'elephant')
    print((str("Position of 'fox': ") + str(fox_position)))
    print((str("Position of 'the': ") + str(the_position)))
    print((str("Position of 'elephant': ") + str(missing_position)))
    space_count = ml_string.count(text, ' ')
    e_count = ml_string.count(text, 'e')
    print((str('Number of spaces: ') + str(space_count)))
    print((str("Number of 'e' characters: ") + str(e_count)))
...
def string_split_join_operations():
    ...
    print((str('Sentences: ') + str(sentences)))
...
def string_validation_operations():
    ...
    test_strings = [empty_string, whitespace_string, alpha_string, numeric_string, alphanumeric_string, mixed_string]
    string_names = ['empty', 'whitespace', 'alpha', 'numeric', 'alphanumeric', 'mixed']
    i = 0
    while (i < test_strings['length']()):
        ...
        print((str('  Empty: ') + str(is_empty)))
        print((str('  Whitespace: ') + str(is_whitespace)))
        print((str('  Alpha: ') + str(is_alpha)))
        print((str('  Numeric: ') + str(is_numeric)))
        print((str('  Alphanumeric: ') + str(is_alnum)))
        i = (i + 1)
...
def regex_operations():
    ...
    print((str('Extracted phone numbers: ') + str(phone_numbers)))
    test_urls = ['https://www.example.com', 'http://test.org', 'ftp://invalid.url', 'not-a-url']
    print('\\nURL Validation:')
    j = 0
    while (j < test_urls['length']()):
        url = test_urls[j]
        is_valid_url = regex['is_url'](url)
        print((str((str((str("'") + str(url))) + str("' is valid URL: "))) + str(is_valid_url)))
        j = (j + 1)

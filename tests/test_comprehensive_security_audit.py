...
eval(processed)
    ...
    for i, test_code in enumerate(injection_tests, 1):
        ...
        # Count detections
        total_detections = (
            len(pattern_matches) + len(violations) + len(flow_results.get("violations", []))
        )
        ...
        if total_detections > 0:
            ...
            print("[MISSED] No threats detected")
...
def test_capability_system_security():
    ...
    file_token = create_file_capability(patterns=["/tmp/*", "*.txt"], operations={"read", "write"})
...
"""
    ...
    print(
        f"  Performance improvement: {improvement_percentage:.1f}% faster with parallel processing"
    )
    ...
    print(
        f"  Cache statistics: {cache_stats['hit_rate']:.1f}% hit rate ({cache_stats['cache_hits']} hits, {cache_stats['cache_misses']} misses)"
    )
    ...
    print(
        f"    Overhead percentage: {overhead_percentage:.1f}% (target: <{target_overhead_percent}%)"
    )

    performance_ok = (
        overhead_ms <= target_overhead_ms and overhead_percentage <= target_overhead_percent
    )
...
def generate_comprehensive_report(results):
    ...
    print("   Version: mlpy v2.0.0 Phase 1")
    ...
    avg_prevention_rate = (
        code_injection_score + import_security_score + reflection_security_score
    ) / 3
